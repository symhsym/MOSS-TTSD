...
class StreamingPackedDataset(IterableDataset):
    ...
    def __init__(
        ...
        self.pkls = [os.path.join(data_dir, f) for f in os.listdir(data_dir) if f.endswith(".pkl")]
        if not self.pkls:
            raise ValueError(f"No .pkl found in {data_dir}")
        if self.shuffle_files:
            rnd = random.Random(self.seed)
            rnd.shuffle(self.pkls)
        
        # Calculate the total number of samples
        self.total_samples = 0
        for pkl_path in self.pkls:
            metas_path = pkl_path.replace(".pkl", "_metas.npy")
            metas = np.load(metas_path)
            pointers = metas[0].astype(np.int64)
            self.total_samples += len(pointers)

        # Estimate the average number of samples per pack (adjustable)
    ...
    def _iter_sample_pointers(self, worker_slice_pkls):
        """Yield (pkl_path, start_pointer_array); shard between workers to avoid duplicate reading."""
        rnd = random.Random(self.seed)
        for pkl_path in worker_slice_pkls:
            metas_path = pkl_path.replace(".pkl", "_metas.npy")
            metas = np.load(metas_path)
            pointers = metas[0].astype(np.int64)
            ...
            if self.shuffle_within_file:
                ptrs = list(pointers)
                rnd.shuffle(ptrs)
                pointers = np.array(ptrs, dtype=np.int64)

            yield pkl_path, pointers
    ...
    def __iter__(self):
        """
        Sharding for multiple workers:
        - Shard by file dimension to each worker;
        - If you want to further refine to pointer-level sharding, you can continue to do modulo allocation in _iter_sample_pointers.
        """
        worker = get_worker_info()
        if worker is None:
            slice_pkls = self.pkls
        else:
            # Simply slice the file list for each worker
            slice_pkls = self.pkls[worker.id::worker.num_workers]

        # Chain together the files and pointers assigned to this worker
        def samples():
            for pkl_path, pointers in self._iter_sample_pointers(slice_pkls):
                yield from self._iter_stream(pkl_path, pointers)

        # Pack samples online and yield
